<template>
  <div class="row d-flex justify-content-between">
    ...
    <div class="mb-3 col-sm-4 d-flex justify-content-center align-items-center">
      <button type="button" class="btn shadow" :data-bs-toggle="this.is_home ? 'modal' : ''" data-bs-target="#img-upload">
        <img class="rounded-circle img-fluid" :src="`/images/profile/${this.user_image}`" data-holder-rendered="true">
      </button>
    </div>
    <div class="mb-3 col-sm-4 text-center">
  ...
  <!-- Modal -->
  <picture-uploader :user_id="this.user_id" @set-img="showErrors"></picture-uploader>
</template>
  ...
  methods: {
    ...
    getCounts() {
      axios.get("/getcounts/" + this.user_id).then((response) => {
        this.update(response.data[0], "posts");
        this.update(response.data[1], "comments");
      });
    },
